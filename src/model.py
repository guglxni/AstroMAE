...
# Util function for random masking
def random_masking(x, mask_ratio):
    """
    Perform per-sample random masking by selecting a subset of patches.
    Args:
        x: [N, L, D], where N is batch size, L is seq len, D is embedding dim
        mask_ratio: ratio of patches to mask
    Returns:
        x_masked: [N, L * (1 - mask_ratio), D]
        mask: [N, L], 0 is keep, 1 is remove
        ids_restore: [N, L], ids to restore original image
    """
...
class MaskedAutoencoder(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768, depth=12, num_heads=12,
        ...
        self.patch_embed = timm.models.vision_transformer.PatchEmbed(img_size, patch_size, in_chans, embed_dim)
...
# Helper function to get 2D sin-cos position embedding
import numpy as np

def get_2d_sincos_pos_embed(embed_dim, grid_size, cls_token=False):
    """
    grid_size: int of the grid height and width
    return: numpy array [grid_size*grid_size, embed_dim] or [1+grid_size*grid_size, embed_dim] (w/ or w/o cls_token)
    """
